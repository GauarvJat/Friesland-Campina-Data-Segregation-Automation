...
# ── Safe imports — show a helpful error if packages are missing ───────────────
_missing = []
try:
    import pandas as pd
except ImportError:
...
if _missing:
    st.error(
        ...
        "This usually means `requirements.txt` is missing from your GitHub repo root "
        "or was not committed. Please ensure your repo contains a file named exactly "
        "`requirements.txt` in the **root folder** (same level as this `.py` file) "
        ...
        "et-xmlfile==1.1.0\n"
        "```\n\n"
        "After adding/committing the file, Streamlit Cloud will reboot and install "
